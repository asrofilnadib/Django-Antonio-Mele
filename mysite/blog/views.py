...
def post_share(request, post_id):
    ...
    if request.method == 'Post':
        # Form telah disubmitted
        form = EmailPostForm(request.POST)
        if form.is_valid():
            # Pengisian form telah melalui validasi
            cd = form.cleaned_data
            # .... send email
            post_url = request.build_absolute_uri(post.get_absolute_url())
            subject = f"{cd['name']} rekomen buat anda baca" \
                      f"{post.title}"
            massage = f"Read {post.title} at {post_url}\n\n" \
                      f"{cd['name']}\'s comments {cd['commments']}"
            send_mail(subject, massage, 'user1@zonetcode.com',
                      [cd['to']])
            sent = True
    else:
        form = EmailPostForm()
